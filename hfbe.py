...
class My_loss(torch.nn.Module):
    def __init__(self, threadhold):
        super().__init__()
        self.threadhold = threadhold
        
    def forward(self, input, target): 
    
        # temp = (input - target)[target>=self.threadhold]
        # loss1 = abs(((temp>0)*2-1).type(torch.float32).mean())
        
        loss1 = ((input[target>=self.threadhold].mean() - target[target>=self.threadhold].mean())**2).mean()
    
        loss2 = ((input - target)**2).mean()
...
for ID_i in range(0,len(all_stationID)):
    ...
    for time_i in range(1,6):
        ...
        MSELoss = My_loss(pd.Series(ytrain01.reshape(-1,)).quantile(0.9))
        ...
        torch.save(bestParam, r'F:\jupyter_notebook\model_dl\result\lstm_highLevelBalance\parameter\%s_%s.pth'%(stationID, time_i))
        ...
        if best_NSE<evaluation_indicators(ytest,ytest_)[4]:
            ...
            torch.save(bestParam, r'F:\jupyter_notebook\model_dl\result\lstm_highLevelBalance\parameter\%s_bestNSE.pth'%(stationID))
            
            with open(r'F:\jupyter_notebook\model_dl\result\lstm_highLevelBalance\%s.json'%(stationID), 'w') as f:
