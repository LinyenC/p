# -*- coding: utf-8 -*-
"""
Created on Sat Aug 20 22:40:29 2022
...
for ID_i in range(0,len(all_stationID)):
    ...
    allData.index = pd.date_range('1982-01-01','2018-12-31', freq='1D')
    ...
    for time_i in range(1,6):
        ...
        lradjust = torch.optim.lr_scheduler.StepLR(opt, step_size=EPOCH//3, gamma=1)  # gamma设成1，则学习率始终不变
        ...
        torch.save(bestParam, r'F:\jupyter_notebook\model_dl2\result\lstm_stdNormalization\parameter\%s_%s.pth'%(stationID, time_i))
        ...
        if best_NSE<evaluation_indicators(ytest,ytest_)[4]:
            ...
            torch.save(bestParam, r'F:\jupyter_notebook\model_dl2\result\lstm_stdNormalization\parameter\%s_bestNSE.pth'%(stationID))
            with open(r'F:\jupyter_notebook\model_dl2\result\lstm_stdNormalization\%s.json'%(stationID), 'w') as f:
